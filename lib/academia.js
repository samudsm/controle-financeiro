...
// ---------------- BI-SET / SUPERSÉRIE ----------------
// Dois (ou mais) exercícios com o mesmo "superset_grupo" formam um bloco:
// você emenda um no outro e só descansa no fim do bloco.

// Junta um exercício da ficha com o seguinte.
export async function agruparComProximo(ficha, fichaExercicioId) {
  garantir();
  const lista = ficha.exercicios;
  const i = lista.findIndex((e) => e.id === fichaExercicioId);
  if (i < 0 || i === lista.length - 1) return null;

  const atual = lista[i];
  const proximo = lista[i + 1];

  // Reaproveita um grupo já existente entre os dois; senão cria o próximo número.
  const usados = lista.map((e) => e.superset_grupo).filter((g) => g != null);
  const grupo =
    atual.superset_grupo ?? proximo.superset_grupo ?? (usados.length ? Math.max(...usados) + 1 : 1);

  await Promise.all([
    atualizarFichaExercicio(atual.id, { superset_grupo: grupo }),
    atualizarFichaExercicio(proximo.id, { superset_grupo: grupo }),
  ]);
  return grupo;
}

// Tira o exercício do bloco. Se sobrar só um no grupo, desfaz o grupo inteiro.
export async function desagrupar(ficha, fichaExercicioId) {
  garantir();
  const alvo = ficha.exercicios.find((e) => e.id === fichaExercicioId);
  if (!alvo?.superset_grupo) return;

  const irmaos = ficha.exercicios.filter(
    (e) => e.superset_grupo === alvo.superset_grupo && e.id !== alvo.id
  );

  await atualizarFichaExercicio(alvo.id, { superset_grupo: null });
  if (irmaos.length === 1) {
    await atualizarFichaExercicio(irmaos[0].id, { superset_grupo: null });
  }
}

// Agrupa exercícios consecutivos que compartilham o mesmo superset_grupo.
// Devolve blocos: [{ grupo, exercicios: [...] }]
export function montarBlocos(exercicios = []) {
  const blocos = [];
  for (const ex of exercicios) {
    const ultimo = blocos[blocos.length - 1];
    if (ex.superset_grupo != null && ultimo?.grupo === ex.superset_grupo) {
      ultimo.exercicios.push(ex);
    } else {
      blocos.push({ grupo: ex.superset_grupo ?? null, exercicios: [ex] });
    }
  }
  return blocos;
}
...
export async function iniciarSessaoDeFicha(fichaId) {
  ...
  if (ficha.exercicios.length) {
    const linhas = ficha.exercicios.map((e, i) => ({
      ...
      observacoes: e.observacoes,
      // O agrupamento de bi-set/supersérie acompanha a sessão, para o
      // histórico lembrar que os exercícios foram feitos emendados.
